...
root.title("BrillouinView")

# This ensures the program exits fully when window closed
def on_closing():
    sys.exit()      
...
def fit_selected_model(event=None):
    ...
    if calibration is not None:
        ...
        #ax1.set_title("Calibration", loc='center', fontsize=12)
        ...
        messagebox.showerror("Error", "No calibration data available. Please load a calibration file.")
...
def fit_selected_model2(event=None):
    ...
    if velocity_x is not None: 
        ...
        axINSET.annotate(f'Chi: {para_chi_value}°', xy=(0.95, 0.95), xycoords='axes fraction',fontsize=10, ha='right', va='top')
        canvasINSET.draw()
        ...
        messagebox.showerror("Error", "No data available. Please load a new file.")
...
def subtract_background():
    ...
    ax2.set_xlabel('Velocity [m/s]', fontsize=12)
    ax2.set_ylabel('Intensity [a.u.]', fontsize=12)
    #ax2.set_title('Data', loc='center', fontsize=12)
    ax2.legend(loc='upper left', fontsize=6)
    ax2.tick_params(axis='x', labelsize=12)
    ax2.tick_params(axis='y', labelsize=12)
    update_axis_bounds()
...
elastic_paned_window.add(center_frame, width=200, height=230) #180
...
def on_fitmodel_clicked():
    ...
    if mineral_type is not None and adjusted_matrix is not None:
        ...
        messagebox.showerror("Error", "Model structure returned an error.")
...
def generate_sparse_test_data(mineral_type,Cijs,offset,start_angle,end_angle,density_value, hkl = None,xyz_norm = None,n_data = 15,noise_mean = 1,noise_sigma = 0.1,error_est = False):
    ...
    if xyz_norm is None and hkl is None:
        ...
        messagebox.showerror("Error", "Please specify start direction in either cartesian coordinates (xyz_norm=) or Miller indices (hkl_norm)")
        return -1
    if hkl is not None:
        plane_normal = hkl
    else:
...
def fit_Cij_lsq(x_vps, vps, x_vs1s, vs1s, x_vs2s, vs2s, theta_0, mineral_type, density_value, result_text, selected_value, xyz_norm=None, hkl_norm=None):
    ...
    # below, set verbose = 1 if want to see optimization details
    result = scipy.optimize.least_squares(fun = obj_fun_complete,x0 = theta_0,bounds = (low_bound, high_bound),verbose = 0,max_nfev=1e15,ftol = 1e-6, xtol = 1e-6)
    ...
    #print(f"Type of std_deviation: {type(std_deviation)}")
    #print(f"Contents of std_deviation before calling create_cij_matrix: {std_deviation}")

    # Define a function to create a 6x6 matrix using cij_elements and result_values
    def create_cij_matrix(structure_elements, values, std_deviation):
        cij_matrix = np.zeros((6, 6))
        error_matrix = np.zeros((6, 6)) if std_deviation is not None else None

        #print(f"Number of structure elements: {len(structure_elements)}")
        #print(f"Number of values: {len(values)}")
        #print(f"Number of standard deviations: {len(std_deviation)}")
        #print(f"Standard deviations: {std_deviation}")
            ...
            for j in range(6):
                ...
                if cij_index in structure_elements:
                    ...
                    if std_deviation is not None:
                        error_matrix[i][j] = std_deviation[structure_elements.index(cij_index)]/ 1e9

        return cij_matrix, error_matrix

    
    structure_elements = cij_elements[selected_value]
    updated_values_padded, error_matrix = create_cij_matrix(structure_elements, result_values, std_deviation)
    np.set_printoptions(precision=2, suppress=True)
    result_text.delete("1.0", tk.END)

    # Display both matrices
    result_text.insert("1.0", "Cij Matrix:\n")
    result_text.insert(tk.END, np.array2string(updated_values_padded))

    if error_matrix is not None:
        result_text.insert(tk.END, "\n\nError Matrix:\n")
        result_text.insert(tk.END, np.array2string(error_matrix))
    ...
    # Return both result and standard deviation (or error)
...
def model_velocity(mineral_type, cij_values):
    ...
    for item in hkl_tree.get_children():
        ...
        if not any(val is None for val in values):
            ...
            errors.append(err)
            #print(f"Error in model_velocity: {err}") 
            #print(f"fit_results in model_velocity: {fit_results}") 
    ...
    fig3.clf()
    ax_vp, ax_vs = fig3.subplots(2, 1, sharex=True)
    colors = plt.get_cmap('tab10', len(all_hkl_labels))
    ...
    for i, (fit_result, hkl) in enumerate(zip(fit_results, all_hkl_labels)):
        ...
        # Plot the fit results for the current hkl combination with the same color for all three sets
        line_vp, = ax_vp.plot(all_angles_fit[i], all_vps_fit[i], '-', linewidth=1, color=color, label=f"{hkl_value}")
        line_vs1, = ax_vs.plot(all_angles_fit[i], all_vs1s_fit[i], '--', linewidth=1, color=color, label="Vs1")
        line_vs2, = ax_vs.plot(all_angles_fit[i], all_vs2s_fit[i], ':', linewidth=1, color=color, label="Vs2")

        # Add labels to the legend of ax_vs
        if i == 0:  # Add the first entry for each set
            lines_vs.extend([line_vs1, line_vs2])
            labels_vs.extend(["Vs1", "Vs2"])
        
        # Collect lines and labels for legend of ax_vp
        lines_vp.append(line_vp)
        labels_vp.append(f"{hkl_value}")
        #print(f"Collected labels for ax_vp: {labels_vp}")
    ...
    ax_vp.set_ylabel('Vp [m/s]', fontsize=12)
    ax_vs.set_ylabel('Vs [m/s]', fontsize=12)
    ax_vs.set_xlabel('Chi [°]', fontsize=12)

    plt.setp(ax_vp.get_xticklabels(), fontsize=12)
    plt.setp(ax_vp.get_yticklabels(), fontsize=12)
    plt.setp(ax_vs.get_xticklabels(), fontsize=12)
    plt.setp(ax_vs.get_yticklabels(), fontsize=12)

    # Add a permanent legend in ax_vs
    legend_font = FontProperties(size=10)  
    legend_vp = ax_vp.legend(loc='upper left', fontsize=legend_font.get_size())  # Legend for Vp and hkl values
    legend_vs = ax_vs.legend(lines_vs, labels_vs, loc='upper right', fontsize=legend_font.get_size())  # Legend for Vs1 and Vs2

    # Enable interactive dragging of legends
    if legend_vp:
        legend_vp.set_draggable(True)
    if legend_vs:
        legend_vs.set_draggable(True)

    # Call the function to calculate elastic properties
            ...
            for line in file:
                ...
                if len(hkl_values) == 3:
                    try:
                        ...
                        hkl_values = [float(val) for val in hkl_values]
                        hkl_tree.insert("", "end", values=hkl_values)
                    except ValueError as e:
                        messagebox.showerror("Error", f"Invalid line in file: {line}\nDetails: {e}")
...
def load_velocity_data():
    ...
    file_path = filedialog.askopenfilename(filetypes=[("CSV files", "*.csv"), ("Text files", "*.txt"), ("Excel files", "*.xlsx")])
    if not file_path:
    ...
    try:
        if file_path.endswith('.csv') or file_path.endswith('.txt'):
            velocity_data = pd.read_csv(file_path, comment='#')
            # If successful, update button appearance
            loadvelocitybutton.config(text="Loaded!", bg="lightgreen", fg="black")
        elif file_path.endswith('.xlsx'):
            velocity_data = pd.read_excel(file_path)
            loadvelocitybutton.config(text="Loaded!", bg="lightgreen", fg="black")
        else:
            #print("Unsupported file format. Please use CSV, TXT, or Excel.")
            messagebox.showerror("Error", "Unsupported file format. Please use CSV, TXT, or Excel.")
            loadvelocitybutton.config(text="Load Vel.", bg="red", fg="white")
            return
    except Exception as e:
        #print("Error reading file:", e)
        messagebox.showerror("Error", f"Failed to load velocity data: {str(e)}")
        return

    # Ensure the required columns are present
    expected_columns = ['Chi', 'Vp', 'Vs1', 'Vs2', 'uvz']
    if not all(column in velocity_data.columns for column in expected_columns):
        #print("File does not contain the required columns.")
        messagebox.showerror("Error", "File does not contain the required columns.")
        return
...
def reverse_modeling(mineral_type, cij_values):
    ...
    if velocity_data is None:
        # Use messagebox to show an error popup
        messagebox.showerror("Error", "No velocity data loaded.")
        return
    ...
    end_angle = float(end_entry.get())
    density_value = float(density_entry.get())
    velocity_data['uvz_tuples'] = velocity_data['uvz'].apply(lambda uvz: tuple(map(float, uvz.split(','))))
    unique_uvz_tuples = velocity_data['uvz_tuples'].unique()
    ...
    for uvz_tuple in unique_uvz_tuples:
        ...
        # Get offset value from the entry field, use a random value if empty
        offset_input = offset_entry.get()
        if offset_input:
        ...
        subset = velocity_data[velocity_data['uvz_tuples'] == uvz_tuple]
        ...
        all_vs2s.append(vs2s)
        """
        # Check and append angles_vps and vps
        if 'angles_vps' in locals() and angles_vps is not None:
            all_angles_vps.append(angles_vps)
        else:
            all_angles_vps.append([])  # Append an empty list or handle it as you prefer

        if 'vps' in locals() and vps is not None:
            all_vps.append(vps)
        else:
            all_vps.append([])

        # Check and append angles_vs1s and vs1s
        if 'angles_vs1s' in locals() and angles_vs1s is not None:
            all_angles_vs1s.append(angles_vs1s)
        else:
            all_angles_vs1s.append([])

        if 'vs1s' in locals() and vs1s is not None:
            all_vs1s.append(vs1s)
        else:
            all_vs1s.append([])

        # Check and append angles_vs2s and vs2s
        if 'angles_vs2s' in locals() and angles_vs2s is not None:
            all_angles_vs2s.append(angles_vs2s)
        else:
            all_angles_vs2s.append([])

        if 'vs2s' in locals() and vs2s is not None:
            all_vs2s.append(vs2s)
        else:
            all_vs2s.append([])
    ...
    # Create lists to hold lines and labels for legends
    lines_vp, labels_vp = [], []
    lines_vs, labels_vs = [], []
    vs1_added, vs2_added = False, False

    # Initialize variables for RMS calculation
    rms_vp_total = 0
    rms_vs1_total = 0
    rms_vs2_total = 0
    num_data_points_vp = 0
    num_data_points_vs1 = 0
    num_data_points_vs2 = 0
    ...
    for i, (fit_result, hkl) in enumerate(zip(fit_results, all_hkl_labels)):
        ...
        line_data_vp, = ax_vp.plot(all_angles_vps[i], all_vps[i], '.', markersize=5, color=color, label=f"Data {hkl_value}")
        line_fit_vp, = ax_vp.plot(all_angles_fit[i], all_vps_fit[i], '-', linewidth=1, color=color)
        lines_vp.extend([line_data_vp, line_fit_vp])
        labels_vp.extend([f"Data {hkl_value}", f"Vp {hkl_value}"])

        # Plot the data for Vs1 and Vs2
        if not vs1_added:
            line_data_vs1, = ax_vs.plot(all_angles_vs1s[i], all_vs1s[i], '.', markersize=5, color=color, label=f"Vs1 {hkl_value}")
            vs1_added = True
        else:
            line_data_vs1, = ax_vs.plot(all_angles_vs1s[i], all_vs1s[i], '.', markersize=5, color=color)
        
        if not vs2_added:
            line_data_vs2, = ax_vs.plot(all_angles_vs2s[i], all_vs2s[i], '.', markersize=5, color=color, label=f"Vs2 {hkl_value}")
            vs2_added = True
        else:
            line_data_vs2, = ax_vs.plot(all_angles_vs2s[i], all_vs2s[i], '.', markersize=5, color=color)
        
        # Plot the fit results for Vs1 and Vs2
        ax_vs.plot(all_angles_fit[i], all_vs1s_fit[i], '--', linewidth=1, color=color)
        ax_vs.plot(all_angles_fit[i], all_vs2s_fit[i], ':', linewidth=1, color=color)
        ...
        # Calculate RMS
        if len(all_vps[i]) > 0:  # Ensure data is available
            interp_func_vp = interp1d(all_angles_fit[i], all_vps_fit[i], kind='linear', fill_value="extrapolate")
            fitted_vp_at_true_angles = interp_func_vp(all_angles_vps[i])

            # Calculate the difference and RMS for Vp
            diff_vp = np.array(all_vps[i]) - fitted_vp_at_true_angles
            rms_vp = np.sqrt(np.mean(diff_vp ** 2))
            rms_vp_total += rms_vp * len(all_vps[i])
            num_data_points_vp += len(all_vps[i])

        #print(f"True Vp: {all_vps[i]}")
        #print(f"Interpolated Fitted Vp: {fitted_vp_at_true_angles}")
        #print(f"Difference: {diff_vp}")
        #print(f"RMS Vp: {rms_vp}")
            
        if len(all_vs1s[i]) > 0:  # Ensure data is available
            interp_func_vs1 = interp1d(all_angles_fit[i], all_vs1s_fit[i], kind='linear', fill_value="extrapolate")
            fitted_vs1_at_true_angles = interp_func_vs1(all_angles_vs1s[i])

            diff_vs1 = np.array(all_vs1s[i]) - fitted_vs1_at_true_angles
            rms_vs1 = np.sqrt(np.mean(diff_vs1 ** 2))
            rms_vs1_total += rms_vs1 * len(all_vs1s[i])
            num_data_points_vs1 += len(all_vs1s[i])

        if len(all_vs2s[i]) > 0:  # Ensure data is available
            interp_func_vs2 = interp1d(all_angles_fit[i], all_vs2s_fit[i], kind='linear', fill_value="extrapolate")
            fitted_vs2_at_true_angles = interp_func_vs2(all_angles_vs2s[i])

            diff_vs2 = np.array(all_vs2s[i]) - fitted_vs2_at_true_angles
            rms_vs2 = np.sqrt(np.mean(diff_vs2 ** 2))
            rms_vs2_total += rms_vs2 * len(all_vs2s[i])

    # Final RMS calculations (averaged over all data points)
    if num_data_points_vp > 0:
        rms_vp_total /= num_data_points_vp
    if num_data_points_vs1 > 0:
        rms_vs1_total /= num_data_points_vs1
    if num_data_points_vs2 > 0:
        rms_vs2_total /= num_data_points_vs2
    ...
    plt.setp(ax_vp.get_xticklabels(), fontsize=12)  # Set x-axis tick label font size
    plt.setp(ax_vp.get_yticklabels(), fontsize=12)  # Set y-axis tick label font size
    plt.setp(ax_vs.get_xticklabels(), fontsize=12)  # Set x-axis tick label font size
    plt.setp(ax_vs.get_yticklabels(), fontsize=12)  # Set y-axis tick label font size

    # Add legends for both axes
    legend_font = FontProperties(size=10)  # FontProperties object to set font size
    legend_vp = ax_vp.legend(lines_vp, labels_vp, loc='upper left', fontsize=legend_font.get_size())
    
    # Add the RMS value for Vp in the legend (one entry for all Vp)
    rms_vp_label = f"RMS: {rms_vp_total:.2f}"
    lines_vp_rms, = ax_vp.plot([], [], ' ', label=rms_vp_label)  # Empty plot to add the RMS to legend
    legend_vp = ax_vp.legend(lines_vp + [lines_vp_rms], labels_vp + [rms_vp_label], loc='upper left', fontsize=legend_font.get_size())

    # Create custom legend handles for ax_vs
    legend_handles_vs = [
        mlines.Line2D([], [], color=colors(0), linestyle='--', linewidth=1, label=f'Vs1 (RMS: {rms_vs1_total:.2f})'),
        mlines.Line2D([], [], color=colors(0), linestyle=':', linewidth=1, label=f'Vs2 (RMS: {rms_vs2_total:.2f})')
    ]
    # Only add legend entries for Vs1 and Vs2 on the first plot
    legend_vs = ax_vs.legend(handles=legend_handles_vs, loc='upper right', fontsize=legend_font.get_size())
...
def save_model():
    ...
    if not save_dir:
        ...
        messagebox.showerror("Error", "No directory selected, saving cancelled.")
        return
...
def load_cij():
    ...
    for line in lines[2:]:  # Skip the first two lines
        ...
        # Skip the "Cij Matrix:" line
        if "Cij Matrix:" in line:
            continue
        
        # Stop when the "Error Matrix:" line is encountered
        if "Error Matrix:" in line:
            break
        ...
        if line.startswith("[[") and line.endswith("]]"):
            line = line[2:-2].strip()  # Remove outer brackets
        elif line.startswith("[") and line.endswith("]"):
            line = line[1:-1].strip()  # Remove single brackets

        # Remove any additional brackets or unexpected characters
        ...
        try:
            ...
            messagebox.showerror("Error", "Format of values in matrix is incorrect.")
            

    # Debugging: Print parsed Cij matrix
    #print("Parsed Cij Matrix:")
    #for row in cij_matrix:
        #print(row)

    # Ensure cij_matrix is a 6x6 matrix
    if len(cij_matrix) != 6 or any(len(row) != 6 for row in cij_matrix):
        #print("Warning: The Cij matrix is not 6x6. Please check the input file.")
        messagebox.showerror("Error", "The Cij matrix is not 6x6. Please check the input file.")
        # Fill the matrix with default values or handle this case as needed
        cij_matrix = [[0] * 6 for _ in range(6)]

    # Populate the entries with the Cij matrix values
    for i in range(6):
        for j in range(6):
            if i < len(cij_matrix) and j < len(cij_matrix[i]):
                entry_value = str(cij_matrix[i][j])
            else:
                entry_value = "0"

            # Ensure that entries[i][j] exists before accessing it
            try:
                entries[i][j].delete(0, tk.END)
                entries[i][j].insert(0, entry_value)
                # Trigger the update_cij_value function to update the internal cij_values
                update_cij_value(i, j)
            except IndexError as e:
                #print(f"Error accessing entry at ({i}, {j}): {e}")
                messagebox.showerror("Error", f"Error accessing entry at ({i}, {j}): {e}")

    # Call the function to update the UI based on the selected structure
    on_structure_selected(None)


fig3, (ax_vp, ax_vs) = plt.subplots(2, 1, sharex=True, figsize=(4.5,3.5))
for ax in (ax_vp, ax_vs):
...
ax_vs.set_ylabel('Vs [m/s]', fontsize=12)
ax_vs.set_xlabel('Chi [°]', fontsize=12)
#ax_vs2.set_ylabel('Vs2 [m/s]', fontsize=12)
...
offset_frame = tk.Frame(center_frame)
offset_frame.pack(side=tk.TOP)
offset_label = tk.Label(offset_frame, text="Offset Value:")
offset_label.pack(side=tk.LEFT)
offset_entry = tk.Entry(offset_frame, width = 3)
offset_entry.pack(side=tk.LEFT)

hkl_tree = ttk.Treeview(center_frame, columns=("U", "V", "Z"), show="headings", height=2)
...
button_row1 = tk.Frame(center_frame)
button_row1.pack(side=tk.TOP, pady=5)  # Pack the first row at the top

load_button = tk.Button(button_row1, text="Load UVZ", command=load_hkl)
load_button.pack(side=tk.LEFT, padx=5)  # Pack on the left side of the first row

fitmodel = tk.Button(button_row1, text="Model Vel.", command=on_fitmodel_clicked)
fitmodel.pack(side=tk.LEFT, padx=5)  # Pack on the left side, next to the first button in row 1

# Second row of buttons
button_row2 = tk.Frame(center_frame)
button_row2.pack(side=tk.TOP, pady=5)  # Pack the second row below the first one

loadvelocitybutton = tk.Button(button_row2, text="Load Vel.", command=load_velocity_data, bg="lightgray")
loadvelocitybutton.pack(side=tk.LEFT, padx=5)  # Pack on the left side of the second row

modelCIJbutton = tk.Button(button_row2, text="Model Cij", command=on_inverse_clicked)
modelCIJbutton.pack(side=tk.LEFT, padx=5)  # Pack on the left side, next to the first button in row 2
...
def update_plotSTACK():
    ...
    if is_calibrated_data_present:
        ax4.set_xlabel('Velocity [m/s]', fontsize=12)
        ax4.set_ylabel('Intensity [a.u.]', fontsize=12)
    else:
        ax4.set_xlabel('Channel Number', fontsize=12)
        ax4.set_ylabel('Intensity [a.u.]', fontsize=12)
